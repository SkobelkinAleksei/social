...
public class LikePostJwtAuthFilter extends OncePerRequestFilter {
    private final LikePostAuthUtil likePostAuthUtil;
    ...
    protected void doFilterInternal(
        ...
        try {
            ...
            String email = likePostAuthUtil.getUsernameFromToken(token);
            Long userId = likePostAuthUtil.getUserIdFromToken(token);

            // Минимальные authorities (для @PreAuthorize)
            Collection<? extends GrantedAuthority> authorities = likePostAuthUtil.getAuthoritiesFromToken(token);
